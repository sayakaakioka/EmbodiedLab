...
def evaluate_grid_policy(
    ...
    """Run deterministic grid-world rollouts and return episode statistics."""
    rewards: list[float] = []
    steps: list[int] = []
    successes = 0
...
def evaluate_policy(
    ...
    """Run deterministic grid-world rollouts and return episode statistics."""
    return evaluate_grid_policy(model=model, env=env, training=training)


def _train_model(
    *,
    env: GridWorldTrainingEnv | ContinuousNavigationEnv,
    training: TrainingConfig,
) -> PPO:
...
def run_gridworld_training(
    ...
    """Train a PPO policy on the grid spec, evaluate it, and optionally save it."""
    ...
    try:
        ...
        evaluation = evaluate_grid_policy(model=model, env=eval_env, training=training)
...
def run_continuous_navigation_training(
    ...
    try:
        ...
        return {
            ...
            "replay_steps": evaluation["replay_steps"],
            "export_onnx": True,
            "model_export_layout": "continuous_navigation",
        }
